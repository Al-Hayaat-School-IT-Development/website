...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Given TASK-003 is complete and the schema is deployed
...
- [ ] #5 Edge case: seed data — Given scripts/db/seed.sql is executed
When the developer runs SELECT COUNT(*) on each seeded table
Then each table returns > 0 rows
- [ ] #6 Edge case: missing table — Given the donations table was not created
