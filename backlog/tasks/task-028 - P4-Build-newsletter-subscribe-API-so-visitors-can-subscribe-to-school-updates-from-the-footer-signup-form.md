...
- Stack constraints: newsletterSchema from TASK-024 (email only), pg Pool singleton from lib/db.ts, ON CONFLICT DO NOTHING for duplicate emails
- Phase dependencies: TASK-024 (newsletterSchema, sendContactConfirmation), TASK-003 (newsletter_subscribers table)
...
- [ ] #5 Edge case: email confirmation fails - Given sendContactConfirmation throws - When the error is caught - Then the error is logged to Application Insights and HTTP 201 is still returned
<!-- AC:END -->
